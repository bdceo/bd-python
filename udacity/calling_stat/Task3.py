...
for call in calls:
    ...
    if call_in.startswith('(080)'):
        if call_out.startswith('('):
            ban_calls.add(call_out[call_out.index('(')+1: call_out.index(')')])
        elif ' ' in call_out:
            ban_calls.add(call_out[0:4])
print('"The numbers called by people in Bangalore have codes:"')
for call in sorted(ban_calls):
    print(call)
